...
while r_lte<=R
    r_lte
    theta_lte(:,:,r_lte) = mcmc_run(y(:,r_lte),x(:,:,r_lte),...
                                    initval(:,r_lte),tau,burnin_lte,...
                                    keep_lte,accrate,seed(r_lte));
...
while r_ilp<=R
    r_ilp
    [ret theta_ilp(:,:,r_ilp) lm] = cqr_ilp(x(:,:,r_ilp),y(:,r_ilp),...
                                            y_c,tau,initval(:,r_ilp), ...
                                            burnin_ilp,keep_ilp);
    if norm(ret)==0
        out = out +1;
    end
